...
export const createNewScene = async (fetchFn: FetchFn, name: string): Promise<CreateNewSceneResponse> => {
    const body = {
        name
    }
...
export const addProductToScene = async (fetchFn: FetchFn, sceneId: number, productId: string): Promise<void> => {
    const body = {
        product: productId
    }
    await fetchFn(`${PUBLIC_BACKEND_URL}/v1/scenes/${sceneId}/products`, {method: 'POST', body: JSON.stringify(body)});
}

export const changeSceneEnvironment = async (fetchFn: FetchFn, sceneId: number, environmentId: string): Promise<void> => {
    const body = {
